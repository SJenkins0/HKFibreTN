...
%DIF DEL ../versions/tex/HKLIFibreTN_v3.6.tex   Thu Oct 23 15:41:52 2025
%DIF ADD ../HKLIFibreTN.tex                     Tue Nov  4 11:32:27 2025
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{placeins}
\usepackage{array}
...
\date{\today\\
{\bf Version v3\DIFdelbegin \DIFdel{.5}\DIFdelend \DIFaddbegin \DIFadd{.7}\DIFaddend }}
...
\DIFaddbegin \item \DIFadd{v3.7 - Add section addressing specific questions from review committee as part of recommendation for switch purchase.
...
\DIFaddbegin \section{\DIFadd{Answers to specific review committee questions}}
\DIFadd{This section is included to answer specific questions asked by the review committee or collaboration in the process of reviewing the system. Where the requested details are not already in this technical note, they will also be added in main body of the text in the appropriate location.
}

\subsection{\DIFadd{Questions included in recommendation on purchasing the Weinert-FG105UCA }[\DIFadd{8x80}] \DIFadd{switch device (24/10/2025)}}

\DIFadd{The following questions are taken from the review committee proposal, and paraphrased where necessary.
}

\subsubsection{\DIFadd{Q1}}
 \begin{quote}
\DIFadd{Measured power transmission with Weinert FP400URT. Power used was $\sim10~\mu$W whereas the typical power of a 375~nm laser pulse is $\sim500000~\mu$W (0.5~W). Can the result ($\sim75\%$) be extrapolated to much more power pulses (factor 50000) expected in HK operation?
 }\end{quote}
\DIFadd{A lot of the issues in recent review iterations seem to result from a confusion between average and peak laser power values, which for pulsed lasers are not the same. Peak power is defined as the maximum instantaneous power emitted from a given laser pulse. The average power is defined as the power from a laser pulse across the given period of repetition, which is dependent on the repetition frequency. As these lasers have such short pulse widths, they are not emitting for the majority of a given time period, and therefore average power is smaller than peak power. The power measurements made and quoted in this technical note will always be the average power values made by optical power meters, as we do not have equipment with the timing resolution to measure the peak power of a single laser pulse.
}

\DIFadd{Taking the 375~nm laser in the question as an example, the laser delivery report in \mbox{%DIFAUXCMD
\cref{fig:375} }\hskip0pt%DIFAUXCMD
measured a pulse energy of 143~pJ emitting a peak power of 595~mW. At a frequency of 40~MHz, the average power delivered is then
}\begin{align*}
    \DIFadd{P_{avg} }&\DIFadd{= E_\text{pulse} \cdot f_{rep} }\\
            &\DIFadd{= 143\times10^{-12} \cdot 40\times10^7 }\\
            &\DIFadd{= 5.72\times10^{-3}~\text{W} }\\
            &\DIFadd{= 5.72~\text{mW}
}\end{align*}
\DIFadd{which is the quoted average power value in the same report. This also roughly matches the online typical laser specification sheets in \mbox{%DIFAUXCMD
\cite{bib:laserhead}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{With this in mind, the difference in average power between the quoted values from the switch tests and the value on specification sheets is a factor of 500, not 50000. This difference is likely related to running the laser at a different frequency, and not at 100\% intensity. The percentage of power transmission is not expected to be affected by increases in power.
}

\subsubsection{\DIFadd{Q2}}
\begin{quote}
\DIFadd{The fiber to be used by the laser system (LI-ID), FG105UCA, has a significantly smaller transmittance than the one used for this measurement FP400URT }[\DIFadd{58\% vs. 82\%, table V, TN-91-v3.5}]\DIFadd{. Any idea/feeling how this fact will deteriorate the result ($\sim$75\%)?
}\end{quote}

\DIFadd{The quoted numbers are taken from the }{\it \DIFadd{preliminary}} \DIFadd{measurements in \mbox{%DIFAUXCMD
\cref{sec:meas:sub:prelim}}\hskip0pt%DIFAUXCMD
. As mentioned there, these measurements were performed with an old version of the LED pulser board and a preliminary 3D printed fibre coupler. Due to the wide output profile of an LED, coupling a fibre to the LED is less efficient for smaller core fibres, and larger variations are expected between different fibres of the same spec, since the connector won't hold the fibre in exactly the same position every time. This most likely accounts for the large difference quoted here.
}

\DIFadd{The attenuation measurements presented in \mbox{%DIFAUXCMD
\cref{sec:atten} }\hskip0pt%DIFAUXCMD
show measured attenuation curves as a function of wavelength, which were made with improved fibre couplers, which allowed for more controlled testing. These results show that the attenuation in FG105UCA at sub-400~nm wavelengths is lower than for FP400URT, and so we shouldn't expect worse transmission from the planned switch. Ultimately, the amount of fibre in these switches in on the order of cm, so will have minimal effect on attenuation anyway.
}

\subsubsection{\DIFadd{Q3}}
\begin{quote}
    {\it \DIFadd{Refers to concern about measurements being made at much lower power, and FG105UCA exhibiting lower transmission}}\\
    \DIFadd{Any idea/feeling if those facts may change significantly the conclusion of negligible cross talk?
}\end{quote}
\DIFadd{The concerns over different power definitions and FG105UCA vs. FP400URT transmission have been addressed already, but we don't expect that residual cross talk, if any, will scale with power. Page 14 of the review committee proposal points out a small number of data points above the baseline noise for the FP400URT cross talk tests, which can be seen in \mbox{%DIFAUXCMD
\cref{fig:weinfp400crosstalkport1,fig:weinfp400crosstalkport2}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{The test was setup with one power meter connected to each of the four output ports, with the software configured to take a power reading from all four every 0.5 seconds. The switch was programmed to switch positions every $\sim10$~s, while the laser was constantly pulsing at a fast rate (can't remember the exact value, but $\mathcal{O}$(kHz)). It can be seen from the mentioned plots that almost all higher points occur at the start or end of a specific port being on. As power meters record the average power over the 0.5~s period, if the switch moves from eg. port 1 to port 2 near the end of that 0.5~s period, the average power recorded on port 2 will be slightly above the baseline noise due to the small number of pulses received. This could happen both from moving between neighbouring ports, and moving across ports to get to others. A similar thing is seen in the GIF50C plots but to a lesser extent, probably due to the much smaller core diameter. If this were genuinely residual cross-talk, we would expect to see roughly constant levels of leaked light as in the Agiltron switch (\mbox{%DIFAUXCMD
\cref{fig:agifp400crosstalkoff}}\hskip0pt%DIFAUXCMD
), not just occasional pulses. We will redo the measurements in a running mode more akin to the LI system expected running to confirm this.
}

\DIFadd{It should also be noted that this was regarded as a stress-test for the system, with a high rate of switching and pulsing. In actual HK running the system will operate at a much slower rate which would avoid this. For the high rate dedicated calibration runs, we would only switch positions once pulsing has stopped. Cross-talk measurements will be made with the final system once we have it, so we have a good understanding of any limitations of the device.
}

\subsubsection{\DIFadd{Questions from P. Urquijo}}
\begin{quote}
    \DIFadd{Is the safety factor in the wear-out lifetime really enough?
}\end{quote}
\DIFadd{We expect to switch at a maximum rate of 0.2~Hz. Assuming constant running (ie. no downtime for other work), this equates to $6.3\times10^6$ switches per year. The switch is rated for up to 1 billion cycles according to quotes we've received, so has an expected lifetime of $10^9/6.3\times10^6 = 159$ HK-years. The switch system will be housed in the calibration rack in the HK dome, so if there are issues before the year 2186 we can easily access the system to replace it.
}

\begin{quote}
    \DIFadd{Will back reflections be an issue?
}\end{quote}
\DIFadd{The switch system itself is small, with a few cm of fibre inside. If there are reflections getting into the detector delayed due to more path length, this will still be less than the level of dispersion from the fibre. The coupling to the laser will also use an FC/APC connection, to mitigate potential issues with reflections getting back to the laser.
}

\begin{quote}
    \DIFadd{Are there any thermal management concerns that might affect couplings?
}\end{quote}
\DIFadd{The specifications recieved from quotes state operating conditions of 0--60}\degree \DIFadd{C and 35--55\% RH. To deal with this the switch (along with other electronics) will be kept in air conditioned racks, so that we can precisely control the environmental conditions.
}
...
\subsection{Preliminary Measurements}\DIFaddbegin \label{sec:meas:sub:prelim}
\DIFaddend 
...
\subsection{Attenuation Measurements}\DIFaddbegin \label{sec:atten}
\DIFaddend 
...
The expected effective attenuation length in \hk is calculated using the factor $R$ and the UV-VIS cell length of $x=9$~cm. Given the worst-case-scenario assumptions in the calculation of $R$, it was found that the effective attenuation length for both samples were above the 2~km limit set by \hk. Details on the calculation of the factor R for each sample soak tested in UPW can be found in Appendix \ref{RCalcUPW} and in gadolinium-doped water in Appendix \ref{RCalcGd}.
...
\begin{itemize}
    \item $R_{LI-Fibre}$ = $\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/89.54\;\text{cm}^3\right)}{\left(1138.79\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $8.99 \times 10^4$
...
\begin{itemize}
    \item $R_{LI-Fibre}$ = $\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/104.74\;\text{cm}^3\right)}{\left(1138.79\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $7.69 \times 10^4$
