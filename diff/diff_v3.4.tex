...
%DIF DEL ../versions/tex/HKLIFibreTN_v3.3.tex   Fri Aug  8 11:32:49 2025
%DIF ADD ../HKLIFibreTN.tex                     Thu Aug 14 17:38:26 2025
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{placeins}
\usepackage{array}
...
\DIFaddbegin \item \DIFadd{v3.4 - Minor revision. Clarify wavelength of LED system and need for wide-core fibre, small section added on LED system which will link to TN when ready. Added specified attenuation plots from Thorlabs.
...
In order to illuminate the injectors, two separate systems will be employed. The first of these will be a system formed of either five or six picosecond pulsed laser sources, in the wavelength range \DIFdelbegin \DIFdel{337-550 }\DIFdelend \DIFaddbegin \DIFadd{355-500 }\DIFaddend nm. This laser system will be used to illuminate all of the ID injectors, along with the 12 OD collimators. The OD diffusers on the other hand will be illuminated by an LED pulser system, utilising a series of 365~nm wavelength LEDs. Both of these systems will feature monitoring devices, so that the stability of the light sources can be monitored prior to convolution with detector parameters.
...
\DIFaddbegin \section{\DIFadd{LED System}}\label{sec:LED}
\DIFadd{Rather than using lasers, the array of 122 OD diffusers will be illuminated with a custom built LED pulser system. Full details of this system are given in }{\bf \DIFadd{reference LED TN once finished}}\DIFadd{. Each pulser board will use a LC LED UT-67UV365P \mbox{%DIFAUXCMD
\cite{bib:LCLED} }\hskip0pt%DIFAUXCMD
surface mounted LED, with a fixed 365~nm wavelength, which will be coupled to a bare fibre end via a 3D printed connector. One pulser board will be used per diffuser. Each connector will also have a second fibre to take light to a set of nearby PMTs for monitoring purposes, for the same purposes as mentioned in \mbox{%DIFAUXCMD
\cref{sec:laser}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{One of the primary purposes of the OD diffuser system is to saturate as many of the \mbox{%DIFAUXCMD
$\sim3600$
}%DIFAUXCMD
PMTs in the OD as possible. As such, the driving factor for both LED and fibre choice for this system is photon yield. Through a series of optimisations in the pulser system electronics, a maximum photon yield of \mbox{%DIFAUXCMD
$2.4\times10^{7}$
}%DIFAUXCMD
photons per pulse was achieved, for a sub-10~ns pulse. This measurement includes attenuation of the signal by 182~m of Thorlabs FP400URT fibre \mbox{%DIFAUXCMD
\cite{bib:fp400urt}}\hskip0pt%DIFAUXCMD
, one of the candidate fibres tested extensively in the following sections. A further \mbox{%DIFAUXCMD
$\sim$
}%DIFAUXCMD
50\% efficiency should also be factored in for the OD diffusers. Saturation studies (}{\bf \DIFadd{these will be in the LED TN)}} \DIFadd{suggest at least \mbox{%DIFAUXCMD
$5.5\times10^6$
}%DIFAUXCMD
photons per pulse are required to have the majority of PMTs above the defined saturation limit of 100 photoelectrons. 
}


\DIFaddend \section{Fibre Lab Measurements}
...
\DIFaddbegin \subsection{\DIFadd{Candidate Fibres}}

\DIFaddend For the fibres to be suitable for use in the LI system, there are a series of requirements. These are:
\begin{itemize}
\item The fibre should be capable of transporting light of wavelengths in the range \DIFdelbegin \DIFdel{337--550}\DIFdelend \DIFaddbegin \DIFadd{355--500}\DIFaddend ~nm.
...
\caption{\DIFaddFL{Thorlabs specified attenuation as a function of wavelength for glass-clad silica fibres including the high hydroxyl content FG050UGA and FG105UCA \mbox{%DIFAUXCMD
\cite{bib:22NAfibres}}\hskip0pt%DIFAUXCMD
.}}\label{fig:105ThorlabsAtten}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{Hard_Clad_Silica_050na_780.png}
\caption{\DIFaddFL{Thorlabs specified attenuation as a function of wavelength for hard-clad silica fibres including the high hydroxyl content FP200URT and FP400URT \mbox{%DIFAUXCMD
\cite{bib:50NAfibres}}\hskip0pt%DIFAUXCMD
.}}\label{fig:400ThorlabsAtten}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{GIF50_SpectralAttenuation_G1-780.png}
\caption{\DIFaddFL{Thorlabs specified attenuation as a function of wavelength for graded index fibres such including GIF50C and GIF50D \mbox{%DIFAUXCMD
\cite{bib:GIfibres}}\hskip0pt%DIFAUXCMD
.}}\label{fig:50CThorlabsAtten}
\end{figure}


\DIFaddend \subsection{Fibre Test Stand}\label{sec:meas:sub:stand}
...
\DIFaddend However, the initial results for the 1~m patch cables given in \cref{tab:attinit} make it clear that coupling an LED to such a small core diameter fibre is \DIFdelbegin \DIFdel{difficult. As }\DIFdelend \DIFaddbegin \DIFadd{less efficient. The next best results in terms of attenuation come from the FP400URT fibre. While the attenuation it exhibited was greater than for the FG105UCA, the four times wider core on this fibre should result in a factor of 16 more input light (assuming same setup otherwise), outweighing the increased attenuation. As was mentioned in \mbox{%DIFAUXCMD
\cref{sec:LED}}\hskip0pt%DIFAUXCMD
, significant work has already been invested into increasing the photon output from the LED pulser system, and the \mbox{%DIFAUXCMD
$2.4\times10^7$
}%DIFAUXCMD
photons per pulse is seen as the maximum possible yield. While this is above the required number for OD PMT saturation mentioned in \mbox{%DIFAUXCMD
\cref{sec:LED}}\hskip0pt%DIFAUXCMD
, half of that difference is taken up by the OD diffuser efficiency and, even if it this was not the case, a factor of 16 loss would result in a photon yield far too low for requirement. As }\DIFaddend such, the choice for the OD LED system is the FP400URT fibre\DIFdelbegin \DIFdel{, which has a four times wider core and the next best attenuation results}\DIFdelend .
...
\DIFaddbegin \bibitem{bib:LCLED} 
\DIFadd{LC LED Corporation, LC LED UT-67UV365P: 0805 UV Chip SMD LED 365nm, }\url{https://www.lc-led.com/products/ut-67uv365p.html}

\DIFaddend %%fibres
\DIFdelbegin \bibitem{bib:fp200urt}
\DIFdelend \DIFaddbegin \bibitem{bib:fp400urt}
\DIFaddend Thorlabs Inc., \DIFdelbegin \DIFdel{FP200URT }\DIFdelend \DIFaddbegin \DIFadd{FP400URT }\DIFaddend Specifications, 6th \DIFdelbegin \DIFdel{February 2024, }%DIFDELCMD < \url{https://www.thorlabs.com/thorproduct.cfm?partnumber=FP200URT}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{December 2022, }\url{https://www.thorlabs.com/thorproduct.cfm?partnumber=FP400URT}
\DIFaddend 


\DIFdelbegin \bibitem{bib:fp400urt}
\DIFdelend \DIFaddbegin \bibitem{bib:fp200urt}
\DIFaddend Thorlabs Inc., \DIFdelbegin \DIFdel{FP400URT }\DIFdelend \DIFaddbegin \DIFadd{FP200URT }\DIFaddend Specifications, 6th \DIFdelbegin \DIFdel{December 2022, }%DIFDELCMD < \url{https://www.thorlabs.com/thorproduct.cfm?partnumber=FP400URT}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{February 2024, }\url{https://www.thorlabs.com/thorproduct.cfm?partnumber=FP200URT}
\DIFaddend 
...
\DIFadd{Thorlabs Inc., 0.22 NA Silica Core, Glass Clad Multimode Optical Fiber, Step Index, MM Fiber Selection, }\url{https://www.thorlabs.com/newgrouppage9.cfm?objectgroup_id=6838}

\bibitem{bib:50NAfibres}
\DIFadd{Thorlabs Inc., 0.50 NA Step-Index Multimode Fibers, MM Fiber Selection, }\url{https://www.thorlabs.com/newgrouppage9.cfm?objectgroup_id=362}

\bibitem{bib:GIfibres}
\DIFadd{Thorlabs Inc., Graded-Index (GRIN) Multimode Fibers, MM Fiber Selection, }\url{https://www.thorlabs.com/newgrouppage9.cfm?objectgroup_id=358}
\DIFaddend 
...
80			&	FC/PC (air)	&	Bare (water)	&	84				&	Barrel			& Bottom OD diff. (all)$^\dagger$	\\
