...
%DIF DEL ../versions/tex/HKLIFibreTN_v3.2.tex   Fri Aug  8 10:57:52 2025
%DIF ADD ../HKLIFibreTN.tex                     Fri Aug  8 10:57:32 2025
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{placeins}
\usepackage{array}
...
\section{\DIFdelbegin \DIFdel{Destructive Testing}\DIFdelend \DIFaddbegin \DIFadd{Additional Fibre Tests}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{sec:destruction}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:tests}
\subsection{\DIFadd{Light Tightness}}\label{sec:tests:sub:light}
\DIFadd{An important quality to test is how light-tight the fibres are to external sources of light, as it is vital that light is only output from the injectors when injection is planned and tagged in the data stream. If the fibres are not light-tight, it is possible that enough light from overhead dome lamps could leak into them to output photons into the detector outside of planned injections. To test this, reels of FP400URT with and without furcation tubing were connected individually to separate power meters. The lengths of these fibres are 35 and 40~m respectively, and both were wound onto reels during the test. The ends not connected to the power meters were terminated in the patch panel on the side of the dark box, to limit any light entering from the fibre ends.
}

\DIFadd{Power meter readings were taken for a total of five minutes, during which the lights in the lab were turned off and on, and a phone torch shone directly at the fibre sides to gauge the impact. The resultant power readings from this can be seen in \mbox{%DIFAUXCMD
\cref{fig:lighttightness}}\hskip0pt%DIFAUXCMD
. The test started with the lights in the lab turned on, and the untubed fibre shows clear light leakage with recorded power values of \mbox{%DIFAUXCMD
$\sim1.8\times10^{-10}$
}%DIFAUXCMD
~W. This is two orders of magnitude higher than from the tubed fibre at \mbox{%DIFAUXCMD
$\mathcal{O}(10^{-12})$
}%DIFAUXCMD
~W, which is essentially just noise.
}\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{PowerVsTime}
\caption{\DIFaddFL{Power measurements as a function of time for two FP400URT fibre optic cables, one with furcation tubing and one without.}}\label{fig:lighttightness}
\end{figure}
\DIFadd{On turning the labs in the light off, the readings from the untubed fibre dropped to be in line with those of the tubed fibre. At around 100~s, with the lab lights turned back on, a phone torch was shone onto the fibres. This induced a small peak in the power readings from both fibres, albeit still significantly less for the tubed fibre. Finally, the torch was shone directly onto the fibres up close. While this didn't cause any further increase in power from the tubed fibre, a sharp peak of nearly \mbox{%DIFAUXCMD
$6\times10^{-10}$
}%DIFAUXCMD
~W is observed from the untubed one.
}
...
\subsection{\DIFadd{Connector Transmission in Water}}\label{sec:tests:sub:water}
\DIFadd{All previous tests have been carried out with the fibres and connectors in air. Due to the nature of Hyper-K, it was therefore important to confirm whether submersion of connected fibres in water has any effect on throughput power. In order to test this, two 1~m FP400URT cables were connected together using a standard adapter. This setup was connected to the laser at one end, and a power meter on the other. After taking data for 3 minutes with the connector in air, it was then submerged into water, and left for continued readings to be taken. The test was also recorded, so the footage could be reviewed and checked for air bubbles when the connected fibres were submerged. A screenshot from that video at the point of submersion is shown in \mbox{%DIFAUXCMD
\cref{fig:watertranstest}}\hskip0pt%DIFAUXCMD
, and at no point during submersion were air bubbles observed to come from the connected fibres.
}\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{submersion}
\caption{\DIFaddFL{Still from the recording of the submersion test, showing the point of submersion. Bubbles visible are on the side of the box, not originating from the connected fibres.}}\label{fig:watertranstest}
\end{figure}
\DIFadd{The data recorded by the power meter as a function of time is shown in \mbox{%DIFAUXCMD
\cref{fig:watertrans}}\hskip0pt%DIFAUXCMD
. It is immediately obvious from this that the power output was not completely stable throughout the test. This is likely to come from two sources, the first being from instability in the laser itself. With such short cable lengths, light reflecting back to the laser is likely to cause instability in the output light.
}\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{PowerVsTimeWater}
\caption{\DIFaddFL{Power measurement as a function of time. The red line shows the point at which the connector with attached fibres was submerged in water.}}\label{fig:watertrans}
\end{figure}
\DIFadd{This is further exacerbated by the second source, which is the fact that the fibres had to be manually handled in order to submerge them. Despite this instability, the maximum global change across the data-taking period is only 1\%.
}

\DIFadd{The red line in \mbox{%DIFAUXCMD
\cref{fig:watertrans} }\hskip0pt%DIFAUXCMD
shows the point at which the connected fibre setup was submerged in water. No obvious decrease in recorded power is observed at or after this point, other than the aforementioned laser instability. Once the test was completed and the fibres were disassembled, the inside of the connector was checked for water, and it was determined there had been no water ingress.
}

\subsection{\DIFadd{Destructive Testing}}\label{sec:tests:sub:destruction}
\DIFaddend Another important aspect of the fibres to test is their durability. While every care should be taken with the fibres to ensure damage does not occur, they will be installed in a working environment, and accidents can happen.  With a large number of fibres of varying length, visually noticing damage to a fibre is very unlikely. As such, it is important to understand what kinds of stress a fibre can realistically be exposed to and what can be tolerated, along with how to quickly check if damage has occurred.
...
\DIFdelbegin \subsection{\DIFdel{Scraping}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Scraping}}
...
With nearly 1000 fibres in total, full characterisation of each one is not viable. Instead, upon delivery of each set of fibres, quick checks of power output through each fibre will be performed with the hand-held testing device detailed in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cref{sec:destruction}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{sec:tests:sub:destruction}}\hskip0pt%DIFAUXCMD
}\DIFaddend . By measuring the transmitted power through all fibres in a set a distribution of values can be built, and the largest outliers excluded down to the number of required fibres. Specification sheets for the FG105UCA \cite{bib:fg105uca} and FP400URT \cite{bib:fp400urt} provide measured attenuation values for a range of wavelengths. By comparing the batch average power to the expectation when accounting for attenuation, any difference should be immediately obvious, and significant issues would be raised with the supplier straight away. During installation of the fibres, power readings with the hand-held devices will be repeated, to ensure no damage has occurred to fibres in the process.
...
\section{Fibre \DIFdelbegin \DIFdel{Length Requirements}\DIFdelend \DIFaddbegin \DIFadd{Configurations}\DIFaddend }\label{sec:lengths}
...
\end{figure}
16 of these fibre ports are distributed roughly evenly around the outer edge of the dome. The full schematic of the dome showing all calibration and fibre ports is given in \cref{fig:ports}. The OD fibre ports in question are those marked with magenta dots and labelled \DIFdelbegin \DIFdel{``odf'' }\DIFdelend \DIFaddbegin \DIFadd{`odf' }\DIFaddend (OD(\begin{CJK*}{UTF8}{min}ファイバー用\end{CJK*})).
\begin{figure}[h]
\centering
...
\end{figure}
\FloatBarrier
In order to limit the effect of potential damage to fibres, these will be split into in-tank and out-of-tank fibres. The fibres laid along the top deck will run from the calibration rack near the centre of the dome to patch panels next to the OD fibre ports. Being on the deck where there will be foot traffic for \DIFdelbegin \DIFdel{up to }\DIFdelend \DIFaddbegin \DIFadd{at least }\DIFaddend 20 years, the out-of-tank fibres are most likely to get damaged. Keeping them separate to the in-tank fibres makes replacing damaged fibres much easier, as it would be implausible to replace the in-tank fibres.

The initial preference for the fibre layout was to keep all fibre lengths the same. While this would involve many injectors being attached to fibres that were unnecessarily long, it would mean that the attenuation, dispersion and timing delays of pulses sent to all injectors would be constant. However, during the process of obtaining quotes, it was discovered that manufacturing a large number of fibres over 100~m would be \DIFdelbegin \DIFdel{problematic }\DIFdelend \DIFaddbegin \DIFadd{difficult }\DIFaddend for the FP400URT, as draw lengths above 100~m are rare. This is problematic as lengths of $\sim$120~m are required to reach from the OD fibre ports to the centre of the bottom end-cap. To work around this, in-tank fibres will be split into two parts, one extending to the bottom of the barrel, and another extending across the bottom end-cap. Connectors will be installed on the bottom corners of the tank to facilitate this.
...
For the ID injectors, the top-end cap will have a single location. The diffuser and collimator will be installed through \DIFdelbegin \DIFdel{the ID fibre port}\DIFdelend \DIFaddbegin \DIFadd{one of the central regular calibration ports}\DIFaddend , labelled `\DIFdelbegin \DIFdel{Bidf}\DIFdelend \DIFaddbegin \DIFadd{Br}\DIFaddend ' in \cref{fig:ports}. Only one position here is needed, as the injectors can easily be removed in the case of any faults. For the bottom end-cap this is of course not possible, and so four sets of injectors will be installed around the centre for \DIFdelbegin \DIFdel{degeneracy}\DIFdelend \DIFaddbegin \DIFadd{contingency}\DIFaddend . A diagram of an end-cap is shown in \cref{fig:endcapmap}. The locations of PMTs and electronics vessels shown will be the same for both top and bottom. Only the ports (shown in yellow) are unique to the top end-cap.
...
For the OD, 19 diffusers will be installed on each end-cap. The locations of these, which are again \DIFaddbegin \DIFadd{the }\DIFaddend same for top and bottom, are shown in \cref{fig:endcapmap}, denoted by cyan stars. These positions are chosen to match as closely as possible to the `ideal' layout shown in \cref{fig:ODdiffmap}. {\bf Exact cell locations will be provided once a cell address scheme is decided upon by integration group}.


\subsection{Fibre Paths}

With the locations of the injectors decided, the routes of fibre optic cables to said injectors can be planned, and the required lengths of each set of fibres obtained. Installation of the majority of fibres will take place after construction of the PMT support structure is completed. A small team of experts will enter the tank in the OD gondola, using this to route fibres around the tank. Given the size of the Hyper-K tank, and the low speed of a gondola (initial designs estimate this at 0.2~ms$^{-1}$), many decisions made here are in an effort to make installation as efficient as possible. \DIFaddbegin \DIFadd{Each section also mentions the connector choices for the respective fibres. Where possible, all underwater connectors should be stainless steel SMA connectors \mbox{%DIFAUXCMD
\cite{bib:SMA}}\hskip0pt%DIFAUXCMD
, to avoid interaction with the ultra-pure (or potentially gadiated) water. Out-of-water connectors have no such requirement, and are therefore set as regular FC/PC connectors.
}\DIFaddend 
...
\end{table}
\end{center}
The maximum length requirement found here is 34572~mm, to get to ODF 11. Adding the 5~m to account for reaching the calibration rack and travelling vertically to the specific crate gives a total distance of 39572~mm. This can then be rounded to \DIFdelbegin \DIFdel{42}\DIFdelend \DIFaddbegin \DIFadd{44}\DIFaddend ~m for safety, and to ensure the fibres have some slack in them. As both ends of the fibres will connect to patch panels \DIFaddbegin \DIFadd{and not be exposed to water}\DIFaddend , each end of the fibre should have an FC/PC connector, for both fibre types.
...
\end{table}
These numbers show that the additional horizontal fibre required to reach from vertical supports to an injector cell is not a problem, as the maximum length case still comes from reaching to the bottom of the barrel. For contingency, and to allow fibres to not be kept taught, the maximum length requirement is rounded up to \DIFdelbegin \DIFdel{76}\DIFdelend \DIFaddbegin \DIFadd{80}\DIFaddend ~m. For the ID system, all fibres will have \DIFaddbegin \DIFadd{an }\DIFaddend FC/PC \DIFdelbegin \DIFdel{connectors on either end}\DIFdelend \DIFaddbegin \DIFadd{connector on the out-of-water end, and a stainless steel SMA connector on the in-water end}\DIFaddend . This is the same for the OD system fibres which traverse the entire barrel depth to the bottom end-cap. For the fibres connecting to OD barrel diffusers however, these will be polished fibre on one end, as this is what has to be pushed into the back of the diffuser hemisphere. \DIFaddbegin \DIFadd{The out-of-water end will have an FC/PC connector to connect to the patch panels near the ports.
}\DIFaddend 
...
1+2			&		\DIFdelbeginFL \DIFdelFL{32	}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{36	}\DIFaddendFL \\
3+4+5		&		\DIFdelbeginFL \DIFdelFL{58	}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{62	}\DIFaddendFL \\
6+7+bottom	&		\DIFdelbeginFL \DIFdelFL{76	}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{80	}\DIFaddendFL \\ \bottomrule
\end{tabular}
...
\subsubsection{Bottom \DIFdelbegin \DIFdel{end-cap}\DIFdelend \DIFaddbegin \DIFadd{End-cap}\DIFaddend }\label{sec:lengths:sub:len:sub:bottom}
...
\end{table}
As would be expected, the greatest length of fibre is needed to reach the diffuser position in the centre, ODEB10. This needs a distance of $18u = 38178$~mm. In order to simplify things, this is rounded to \DIFdelbegin \DIFdel{42}\DIFdelend \DIFaddbegin \DIFadd{44}\DIFaddend ~m, matching the lengths needed for the deck fibres described in \cref{sec:lengths:sub:len:sub:deck}. All of these OD diffuser fibres will need to have an \DIFdelbegin \DIFdel{FC/PC }\DIFdelend \DIFaddbegin \DIFadd{SMA }\DIFaddend connector on one end, and polished bare fibre on the other.

\DIFdelbegin \DIFdel{Not shown here are }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:bottomEndCapID} }\hskip0pt%DIFAUXCMD
shows }\DIFaddend the ID injector positions \DIFaddbegin \DIFadd{on the bottom end-cap}\DIFaddend . Each of the four sets will be positioned in one of the four $3\times 3$ cell squares diagonally adjacent to the central square. The longest \DIFdelbegin \DIFdel{fibre }\DIFdelend \DIFaddbegin \DIFadd{fibres }\DIFaddend for these is \DIFaddbegin \DIFadd{are those }\DIFaddend required to run \DIFdelbegin \DIFdel{from vertical tR33 to the top right injector, measuring a length }\DIFdelend \DIFaddbegin \DIFadd{to ID locations 29 and 30, requiring lengths }\DIFaddend of $19u=40299$~mm. This is still below the decided \DIFdelbegin \DIFdel{42}\DIFdelend \DIFaddbegin \DIFadd{44}\DIFaddend ~m length, so causes no issues. \DIFaddbegin \DIFadd{The lengths for all four positions are given in \mbox{%DIFAUXCMD
\cref{tab:bottomEndCapIDLen}}\hskip0pt%DIFAUXCMD
. }\DIFaddend {\bf Exact cell location will be updated once we have a decided system}. These fibres will require \DIFdelbegin \DIFdel{FC/PC }\DIFdelend \DIFaddbegin \DIFadd{SMA }\DIFaddend connectors on both ends.

\DIFaddbegin \begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{bottomEndCapMapID}
\caption{\DIFaddFL{Map of ID injector positions on bottom end-cap (red crosses), showing fibre routing plan. Schematic shown is for the deck so port positions do not apply.}}\label{fig:bottomEndCapID}
\end{figure}

\begin{table}[h!]
\centering
\begin{tabular}[t]{ccc}\toprule
\DIFaddFL{ID	}&	\DIFaddFL{Vertical	}&	\DIFaddFL{Distance }[\DIFaddFL{\mbox{%DIFAUXCMD
$u$
}%DIFAUXCMD
}]	\\ \midrule
\DIFaddFL{29	}&		\DIFaddFL{65		}&	\DIFaddFL{19		}\\
\DIFaddFL{30	}&		\DIFaddFL{33		}&	\DIFaddFL{19		}\\
\DIFaddFL{31	}&		\DIFaddFL{65		}&	\DIFaddFL{17		}\\
\DIFaddFL{32	}&		\DIFaddFL{52		}&	\DIFaddFL{17		}\\	\bottomrule
\end{tabular}
\caption{\DIFaddFL{Required distance of fibre optic cable to reach each OD end-cap bottom (ODEB) diffuser, in units of 2121~mm. Also shown is the vertical support that the fibre will be routed from the bottom of.}}\label{tab:bottomEndCapIDLen}
\end{table}



\DIFaddend \subsubsection{Top \DIFdelbegin \DIFdel{end-cap}\DIFdelend \DIFaddbegin \DIFadd{End-cap}\DIFaddend }\label{sec:lengths:sub:len:sub:top}

As previously mentioned, the ID injectors for the top end-cap will be deployed through \DIFdelbegin \DIFdel{the Bidf }\DIFdelend \DIFaddbegin \DIFadd{a central `Br' }\DIFaddend calibration port, meaning that they can be easily removed and replaced should the need arise. In this case, only one section of fibre is needed to go from the injectors to the calibration rack. The length of these is chosen as 10~m, well above the 4506~mm height of the described port. \DIFdelbegin \DIFdel{Both ends of these }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend fibres will require FC/PC connectors on \DIFaddbegin \DIFadd{one end, stainless steel SMA on the other}\DIFaddend . This also matches to the length chosen for the top OD collimators.

The OD diffusers on the top end-cap (labelled ODET 1--19) will be installed in exactly the same $x$ and $y$ locations as for the bottom end-cap. The difference is in the fibre routing, as there is no easy way to get the fibres from an OD fibre port back up to the top end-cap frame. Instead, all of these fibres will \DIFdelbegin \DIFdel{also }\DIFdelend be routed through the \DIFdelbegin \DIFdel{Bidf }\DIFdelend \DIFaddbegin \DIFadd{`Bidf' }\DIFaddend calibration port, and then spread out from this position to each respective diffuser. The fibre routing map for this is given in \cref{fig:topEndCap}.
\begin{figure}[h]
\centering
...
\hspace{1cm}
\begin{tabular}[t]{cc}\toprule
\DIFdelbeginFL \DIFdelFL{ODEB	}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ODET	}\DIFaddendFL &		Distance [$u$]	\\ \midrule
11		&		8		\\
12		&		14		\\
13		&		17		\\
14		&		11		\\
...
\DIFdelbeginFL \DIFdelFL{76			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{80			}\DIFaddendFL &	FC/PC		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	73				&	Barrel (all)$^\dagger$	\\
\DIFdelbeginFL \DIFdelFL{42			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{44			}\DIFaddendFL &	FC/PC		&	FC/PC		&	\DIFdelbeginFL \DIFdelFL{84				}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{76				}\DIFaddendFL &	Deck	\DIFdelbeginFL \DIFdelFL{and bottom	}\DIFdelendFL \\
\DIFaddbeginFL \DIFaddFL{44			}&	\DIFaddFL{SMA			}&	\DIFaddFL{SMA			}&	\DIFaddFL{8				}&	\DIFaddFL{Bottom	}\\
\DIFaddendFL 10			&	FC/PC		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	5				&	Top ID inj. and top OD coll. \\
1			&	FC/PC		&	FC/PC		&	8				&	Laser system patch cable \\
0.64		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	FC/PC		&	32				&	ID diffuser patch cable	\\
0.56		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	FC/PC		&	32				&	ID collimator patch cable	\\ \midrule
\multicolumn{3}{l}{$^\dagger$Staged approach:}			&					&			\\
\DIFdelbeginFL \DIFdelFL{32			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{36			}\DIFaddendFL &	FC/PC		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	16				&	Barrel (levels 1+2) \\
\DIFdelbeginFL \DIFdelFL{58			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{62			}\DIFaddendFL &	FC/PC		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	24				&	Barrel (levels 3+4+5)	\\
\DIFdelbeginFL \DIFdelFL{76			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{80			}\DIFaddendFL &	FC/PC		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	33				&	Barrel (levels 6+7+bottom)	\\
\bottomrule
\end{tabular}
...
\toprule
Length [m]	&	Connector A	&	Connector B	&	Required no.	&	Use		\\ \midrule
\DIFdelbeginFL \DIFdelFL{76			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{80			}\DIFaddendFL &	FC/PC		&	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	19				&	Barrel to bottom	\\
\DIFdelbeginFL \DIFdelFL{76			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{80			}\DIFaddendFL &	FC/PC		&	Bare		&	84				&	Barrel injs (all)$^\dagger$	\\
50			&	FC/PC		&	Bare		&	19				&	Top diffusers \\
\DIFdelbeginFL \DIFdelFL{42			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{44			}\DIFaddendFL &	FC/PC		&	FC/PC		&	103				&	Deck		\\
\DIFdelbeginFL \DIFdelFL{42			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{44			}\DIFaddendFL &	\DIFdelbeginFL \DIFdelFL{FC/PC		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{SMA			}\DIFaddendFL &	Bare		&	19				&	Bottom diffusers	\\
...
\multicolumn{3}{l}{$^\dagger$Staged approach:}			&					&			\\
\DIFdelbeginFL \DIFdelFL{32			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{36			}\DIFaddendFL &	FC/PC		&	Bare		&	24				&	Barrel (levels 1+2) \\
\DIFdelbeginFL \DIFdelFL{58			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{62			}\DIFaddendFL &	FC/PC		&	Bare		&	36				&	Barrel (levels 3+4+5)	\\
\DIFdelbeginFL \DIFdelFL{76			}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{80			}\DIFaddendFL &	FC/PC		&	Bare		&	24				&	Barrel (levels 6+7)	\\
\bottomrule
\end{tabular}
...
\DIFaddbegin \bibitem{bib:SMA}
\DIFadd{Thorlabs Inc., 10720V - High-Temp., Vacuum-Compatible SMA905 MM Connector, Ø720 µm Bore, SS Ferrule, }\url{https://www.thorlabs.com/thorproduct.cfm?partnumber=10720V}


\DIFaddend %%photon requirements
\bibitem{bib:wcsim}
...
%DIFDELCMD < 76			&	FC/PC (air)		&	FC/PC (water)		&	56				&	Barrel		&	Barrel ID (all)$^\dagger$	\\
%DIFDELCMD < 76			&	FC/PC (air)		&	FC/PC (water)		&	8				&	Barrel		&	Bottom ID	\\
%DIFDELCMD < 76			&	FC/PC (air)		&	FC/PC (water)		&	9				&	Barrel		&	Bottom edge OD collimators	\\
%DIFDELCMD < 42			&	FC/PC (air)		&	FC/PC (air)		&	56				&	Deck		&	Barrel ID	\\
%DIFDELCMD < 42			&	FC/PC (air)		&	FC/PC (air)		&	8				&	Deck		&	Bottom ID	\\
%DIFDELCMD < 42			&	FC/PC (air)		&	FC/PC (air)		&	12				&	Deck		&	All OD collimators	\\
%DIFDELCMD < 42			&	FC/PC (water)		&	FC/PC (water)		&	8				&	Bottom		&	Bottom ID	\\
%DIFDELCMD < 10			&	FC/PC (air)		&	FC/PC (water)		&	2				&	Top			&	Top ID		\\
%DIFDELCMD < 10			&	FC/PC (air)		&	FC/PC (water)		&	3				&	Top			& 	Top edge OD collimators \\
%DIFDELCMD < 1			&	FC/PC (air)		&	FC/PC (air)		&	8				&	Laser system	&	Before channel split \\
%DIFDELCMD < 0.64		&	FC/PC (water)		&	FC/PC (water)		&	28				&	Barrel (dead region)	& ID diffuser patch cable	\\
%DIFDELCMD < 0.64		&	FC/PC (water)		&	FC/PC (water)		&	4				&	Bottom (dead region)	& ID diffuser patch cable	\\
%DIFDELCMD < 0.56		&	FC/PC (water)		&	FC/PC (water)		&	28				&	Barrel (dead region)	& ID collimator patch cable	\\
%DIFDELCMD < 0.56		&	FC/PC (water)		&	FC/PC (water)		&	4				&	Bottom (dead region)	& ID collimator patch cable	\\ \midrule
%DIFDELCMD < \multicolumn{3}{l}{$^\dagger$Staged approach:}			&					&			\\
%DIFDELCMD < 32			&	FC/PC (air)		&	FC/PC (water)		&	16				&	Barrel	&	Barrel ID (levels 1+2) \\
%DIFDELCMD < 58			&	FC/PC (air)			&	FC/PC (water)		&	24				&	Barrel	&	Barrel ID (levels 3+4+5)	\\
%DIFDELCMD < 76			&	FC/PC (air)			&	FC/PC (water)		&	16				&	Barrel	&	Barrel ID (levels 6+7)	\\
...
80			&	FC/PC (air)		&	SMA (water)		&	9				&	Barrel		&	Bottom edge OD collimators	\\
...
10			&	FC/PC (air)		&	SMA (water)		&	3				&	Top			& 	Top edge OD collimators \\
...
%DIFDELCMD < \toprule	
%DIFDELCMD < Length [m]	&	Connector A	&	Connector B	&	Req. no.		&	Fibre location	& Injector use	\\ \midrule
%DIFDELCMD < 76			&	FC/PC (air)	&	FC/PC (water)	&	19				&	Barrel			& Bottom OD diff.	\\
%DIFDELCMD < 76			&	FC/PC (air)	&	Bare (water)	&	84				&	Barrel			& Bottom OD diff. (all)$^\dagger$	\\
%DIFDELCMD < 50			&	FC/PC (air)	&	Bare (water)	&	19				&	Top				& Top OD diff. \\
%DIFDELCMD < 42			&	FC/PC (air)	&	FC/PC (air)	&	19				&	Deck			& Bottom OD diff. \\
%DIFDELCMD < 42			&	FC/PC (air)	&	FC/PC (air)	&	84				&	Deck			& Barrel OD diff. \\
%DIFDELCMD < 42			&	FC/PC (air)	&	Bare (water)	&	19				&	Bottom			& Bottom OD diff.	\\
...
%DIFDELCMD < \multicolumn{3}{l}{$^\dagger$Staged approach:}			&					&			\\
%DIFDELCMD < 32			&	FC/PC (air)	&	Bare (water)	&	24				&	Barrel &	Barrel (levels 1+2) \\
%DIFDELCMD < 58			&	FC/PC (air)	&	Bare (water)	&	36				&	Barrel &	Barrel (levels 3+4+5)	\\
%DIFDELCMD < 76			&	FC/PC (air)	&	Bare (water)	&	24				&	Barrel &	Barrel (levels 6+7)	\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
80			&	FC/PC (air)	&	Bare (water)	&	84				&	Barrel			& Bottom OD diff. (all)\mbox{%DIFAUXCMD
$^\dagger$
}%DIFAUXCMD
\\
...
Staged approach:}			&					&			\\
36			&	FC/PC (air)	&	Bare (water)	&	24				&	Barrel &	Barrel (levels 1+2) \\
62			&	FC/PC (air)	&	Bare (water)	&	36				&	Barrel &	Barrel (levels 3+4+5)	\\
80			&	FC/PC (air)	&	Bare (water)	&	24				&	Barrel &	Barrel (levels 6+7)	\\
\bottomrule
\end{tabular}
}
