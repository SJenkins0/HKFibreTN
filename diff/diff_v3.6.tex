...
%DIF DEL ../versions/tex/HKLIFibreTN_v3.5.tex   Wed Oct  1 21:28:44 2025
%DIF ADD ../HKLIFibreTN.tex                     Thu Oct 23 15:29:00 2025
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{placeins}
\usepackage{array}
...
\DIFaddbegin \item \DIFadd{v3.6 - Minor revision to add laser delivery reports from PicoQuant and description of expected running conditions.
...
LDH-D-C-375		&	373			&	52						&	\DIFdelbeginFL %DIFDELCMD < \phantom{00}%%%
\DIFdelFL{70	}\DIFdelendFL \DIFaddbeginFL \phantom{0}\DIFaddFL{143	}\DIFaddendFL \\
...
Rather than using lasers, the array of 122 OD diffusers will be illuminated with a custom built LED pulser system. Full details of this system are given in \DIFdelbegin %DIFDELCMD < {\bf %%%
\DIFdel{reference LED TN once finished}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bib:tn0111}}\hskip0pt%DIFAUXCMD
}\DIFaddend . Each pulser board will use a LC LED UT-67UV365P \cite{bib:LCLED} surface mounted LED, with a fixed 365~nm wavelength, which will be coupled to a bare fibre end via a 3D printed connector. One pulser board will be used per diffuser. Each connector will also have a second fibre to take light to a set of nearby PMTs for monitoring purposes, for the same purposes as mentioned in \cref{sec:laser}.

One of the primary purposes of the OD diffuser system is to saturate as many of the $\sim3600$ PMTs in the OD as possible. As such, the driving factor for both LED and fibre choice for this system is photon yield. Through a series of optimisations in the pulser system electronics, a maximum photon yield of $2.4\times10^{7}$ photons per pulse was achieved, for a sub-10~ns pulse. This measurement includes attenuation of the signal by 182~m of Thorlabs FP400URT fibre \cite{bib:fp400urt}, one of the candidate fibres tested extensively in the following sections. A further $\sim$50\% efficiency should also be factored in for the OD diffusers. Saturation studies \DIFdelbegin \DIFdel{(}%DIFDELCMD < {\bf %%%
\DIFdel{these will be in the LED TN)}%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{shown in \mbox{%DIFAUXCMD
\cite{bib:tn0111} }\hskip0pt%DIFAUXCMD
}\DIFaddend suggest at least \DIFdelbegin \DIFdel{$5.5\times10^6$ }\DIFdelend \DIFaddbegin \DIFadd{$10\times10^6$ }\DIFaddend photons per pulse are required to have the majority of PMTs above the defined saturation limit of 100 photoelectrons.
...
80			&	FC/PC		&	SMA			&	\DIFdelbeginFL \DIFdelFL{33				}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{24				}\DIFaddendFL &	Barrel (levels 6+7+bottom)	\\
\bottomrule
\end{tabular}
...
%DIF > OD TN
\DIFaddbegin \bibitem{bib:tn0111}
\DIFadd{B. Bogdan }{\it \DIFadd{et al.}}\DIFadd{, Hyper-Kamiokande Outer Detector Light Injector System Technical Note (HK-TN-0111)
}

\DIFaddend %%laser specs
...
\section{\DIFadd{Laser Delivery Reports}}\label{app:laserdeliv}
\subsection{\DIFadd{355~nm}}\label{app:laserdeliv:sub:355}

\begin{figure}[h!]
\centering
\includegraphics[angle=90,width=0.85\textwidth]{355nm_measured.png}
\caption{\DIFaddFL{Laser delivery report for example PicoQuant LDH-P-FA-355 laser.}}\label{fig:355}
\end{figure}

\clearpage
\newpage
\subsection{\DIFadd{375~nm}}\label{app:laserdeliv:sub:375}

\begin{figure}[h!]
\centering
\includegraphics[angle=90,width=0.85\textwidth]{375_measured.png}
\caption{\DIFaddFL{Laser delivery report for example PicoQuant LDH-D-C-375 laser.}}\label{fig:375}
\end{figure}

\clearpage
\newpage
\subsection{\DIFadd{395~nm}}\label{app:laserdeliv:sub:395}

\begin{figure}[h!]
\centering
\includegraphics[angle=90,width=0.85\textwidth]{390_measured.png}
\caption{\DIFaddFL{Laser delivery report for example PicoQuant LDH-D-C-390 laser.}}\label{fig:390}
\end{figure}

\clearpage
\newpage
\subsection{\DIFadd{440~nm}}\label{app:laserdeliv:sub:440}

\begin{figure}[h!]
\centering
\includegraphics[angle=90,width=0.85\textwidth]{440_measured.png}
\caption{\DIFaddFL{Laser delivery report for example PicoQuant LDH-D-C-440 laser.}}\label{fig:440}
\end{figure}

\clearpage
\newpage
\subsection{\DIFadd{500~nm}}\label{app:laserdeliv:sub:500}

\begin{figure}[h!]
\centering
\includegraphics[angle=90,width=0.85\textwidth]{500_measured.png}
\caption{\DIFaddFL{Laser delivery report for example PicoQuant LDH-D-C-500 laser.}}\label{fig:500}
\end{figure}
\DIFaddend 
