...
%DIF DEL ../versions/tex/HKLIFibreTN_v3.7.tex   Tue Nov  4 20:33:15 2025
%DIF ADD ../HKLIFibreTN.tex                     Sat Nov 15 13:07:14 2025
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{placeins}
\usepackage{array}
...
\usepackage{siunitx}
%DIF 43a43
\DeclareSIUnit{\connector}{connector} %DIF > 
%DIF -------
...
\date{\today\\
{\bf Version v3\DIFdelbegin \DIFdel{.7}\DIFdelend \DIFaddbegin \DIFadd{.8}\DIFaddend }}
...
\DIFaddbegin \item \DIFadd{v3.8 - Add fibre radon measurements. Specify fibre and connector types.
...
The first deployed test of one of the LI optics was performed with WCTE, in which \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend ID diffuser was included as a calibration source. During WCTE decommissioning, it was observed that the FC/PC connector on the end of the FG105UCA fibre had undergone reaction with either the Ultra Pure Water (UPW) or gadolinium-doped water. Along with the need to confirm everything that is placed into Hyper-K is safe for long-term exposure, this observation motivated the soak testing of the fibre and the fibre connectors. Water quality degradation is to be assessed with the UV effective attenuation length weighted by the expected Cherenkov light response of the PMTs.

The samples used in the test were a 10~cm long \DIFdelbegin \DIFdel{fibre and one fibre }\DIFdelend \DIFaddbegin \DIFadd{FP400URT fibre with FT030 furcation tubing, and one }\DIFaddend FC/PC fibre connector. These were initially cleaned with isopropyl alcohol (IPA), before being  placed into a sterile 100~ml PET container with a known volume of deionised water. At the same time, an additional sterile PET container was filled with a control sample of water from the same source. The containers were \DIFdelbegin \DIFdel{be }\DIFdelend immediately sealed, labelled, and placed \DIFdelbegin \DIFdel{into }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend a dark constant environment chamber set at $14^\circ$C. The procedure above was \DIFdelbegin \DIFdel{be }\DIFdelend repeated for another sample of the same material in a solution of 0.2\% Gd$_2$(SO$_4$)$_3\cdot$8H$_2$O in deionised water.

The standard procedure applied for \hk material soak \DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{testing }\DIFaddend is to leave the sample to soak undisturbed for 90 days. This period was estimated conservatively to be the period of water turnover in the \hk water tank. However, given the long lead time for purchase and delivery of the fibre, it was decided to perform a first measurement of the water properties after one month of soaking, before returning the samples to the containers for the remainder of the time.
...
The expected effective attenuation length in \hk is calculated using the factor $R$ and the UV-VIS cell length of $x=9$~cm. Given the worst-case-scenario assumptions in the calculation of $R$, it was found that the effective attenuation length for both samples \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend above the 2~km limit set by \hk. Details on the calculation of the factor R for each sample soak tested in UPW can be found in Appendix \ref{RCalcUPW} and in gadolinium-doped water in Appendix \ref{RCalcGd}.
...
\DIFaddbegin \begin{figure}
   ...
   \caption{\label{fig:LIFibre_2months}\DIFaddFL{\% Transmission: Fibre (after two additional months of soaking)}}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.49\linewidth}
    \centering
    \includegraphics[width=\textwidth]{transmittance_Fibre_End_3Months.png}
    \caption{\label{fig:FibreEnd_2months}\DIFaddFL{\% Transmission: FC/PC connector (after two additional months of soaking)}}
    \end{minipage}
\end{figure}


\subsection{\DIFadd{Radon Testing}}\label{sec:tests:sub:radon}


\DIFadd{Measurements were conducted following the same methodology as used in the Kamioka radon test benches. 
%DIF > 
...
%DIF > 
\DIFadd{To separate the results from the long fibre jacket lengths that will be in HK and the connectors, the end connectors of a ThorLabs fibre were cut off so that
 \mbox{%DIFAUXCMD
\SI{87}{\centi\meter} }\hskip0pt%DIFAUXCMD
length of FP400URT optical fibre with FT030 furcation tubing could be tested alone. These samples are shown in the emanation chambers in \mbox{%DIFAUXCMD
\cref{fig:boulbe_radon_photos}}\hskip0pt%DIFAUXCMD
.
The measured activities of \ce{^{222}Rn} in the fibre jacket were determined from the decay products to be \mbox{%DIFAUXCMD
\SI{2.784 \pm 0.125}{\milli\becquerel} }\hskip0pt%DIFAUXCMD
from \ce{^{218}Po} and \mbox{%DIFAUXCMD
\SI{2.101 \pm 0.095}{\milli\becquerel} }\hskip0pt%DIFAUXCMD
\ce{^{214}Po}, and are shown in \mbox{%DIFAUXCMD
\cref{fig:boulby_rad25064}}\hskip0pt%DIFAUXCMD
.
}
...
\begin{figure}
    ...
    \caption{\DIFaddFL{Optical fibre jacket sample and end connectors in the Boulby emanation chambers.}}
    \label{fig:boulbe_radon_photos}
\end{figure}

\DIFadd{To remain conservative, the higher of the two values from the fibre was used, yielding an upper bound of \mbox{%DIFAUXCMD
\SI{3.2}{\milli\becquerel\per\meter} }\hskip0pt%DIFAUXCMD
of fibre.
%DIF > 
Scaling this result to the full fibre length used in the Hyper-Kamiokande detector gives a total emanation of \mbox{%DIFAUXCMD
\SI{39.21}{\becquerel}}\hskip0pt%DIFAUXCMD
, corresponding to approximately \mbox{%DIFAUXCMD
\SI{9.8}{\percent} }\hskip0pt%DIFAUXCMD
of the total radon budget (assuming the inner detector limit of \mbox{%DIFAUXCMD
\SI{400}{\becquerel} }\hskip0pt%DIFAUXCMD
applied to the outer detector).
}
...
\begin{figure}
    ...
    \caption{\DIFaddFL{Results for the fibre tests at Boulby showing the decay of radon daughters after the initial spike. }}
    \label{fig:boulby_rad25064}
\end{figure}

\DIFadd{Additional measurements were conducted on the new stainless steel SMA connectors from ThorLabs (ADASMAV), which are expected to have improved resistance to gadolinium corrosion. Tests were performed both with and without epoxy sealing to assess the effect the epoxy had on emanation. The epoxy tested was the same known to be used for holding the fibres inside the fibre end connectors, Thorlabs 353NDPK. These are shown in \mbox{%DIFAUXCMD
\cref{fig:boulby_rad25064conn}}\hskip0pt%DIFAUXCMD
. Taking the highest conservative measurement in each case resulted in \mbox{%DIFAUXCMD
\SI{1.031 \pm 0.110}{\milli\becquerel\per\connector} }\hskip0pt%DIFAUXCMD
for the connector without epoxy, and \mbox{%DIFAUXCMD
\SI{0.813 \pm 0.049}{\milli\becquerel\per connector} }\hskip0pt%DIFAUXCMD
for the connector with epoxy.
%DIF > 
For 150 connectors, this corresponds to a total emanation of approximately \mbox{%DIFAUXCMD
\SI{154}{\milli\becquerel} }\hskip0pt%DIFAUXCMD
(\mbox{%DIFAUXCMD
\SI{0.04}{\percent} }\hskip0pt%DIFAUXCMD
of the total radon budget).
}
...
\begin{figure}
    ...
    \caption{\DIFaddFL{Results for the connector tests at Boulby showing the decay of radon daughters after the initial spike. (Left) With epoxy. (Right) Without epoxy. }}
    \label{fig:boulby_rad25064conn}
\end{figure}


\DIFadd{Combining the fibre and connector contributions yields an estimated total radon emanation of:
}\[
\DIFadd{\mbox{%DIFAUXCMD
\SI{39.364 \pm 1.773}{\becquerel} }\hskip0pt%DIFAUXCMD
\quad (\text{approximately } 9.8\% \text{ of budget})
}\]

\begin{table}[h!]
\centering
\caption{\DIFaddFL{Summary of Radon Emanation Measurements}}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{\DIFaddFL{Component}} & \textbf{\DIFaddFL{Measured Activity}} & \textbf{\DIFaddFL{Scaling / Quantity}} & \textbf{\DIFaddFL{Total Contribution}} \\ \midrule
\DIFaddFL{Fibre (\mbox{%DIFAUXCMD
\SI{87}{cm}}\hskip0pt%DIFAUXCMD
) from \ce{^{218}Po} }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{2.784 \pm 0.125}{mBq} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{--- }& \DIFaddFL{--- }\\
\DIFaddFL{Fibre (\mbox{%DIFAUXCMD
\SI{87}{cm}}\hskip0pt%DIFAUXCMD
) from \ce{^{214}Po} }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{2.101 \pm 0.095}{mBq} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{--- }& \DIFaddFL{--- }\\
\DIFaddFL{Conservative fibre activity }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{3.2}{mBq/m} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{12,250 km }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{39.21}{Bq} }\hskip0pt%DIFAUXCMD
}\\
\DIFaddFL{Connector (no epoxy) }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{1.031 \pm 0.110}{mBq/conn.} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{150 units }& \DIFaddFL{--- }\\
\DIFaddFL{Connector (with epoxy) }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{0.813 \pm 0.049}{mBq/conn.} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{150 units }& \DIFaddFL{\mbox{%DIFAUXCMD
\SI{0.154}{Bq} }\hskip0pt%DIFAUXCMD
}\\
\textbf{\DIFaddFL{Total (fibres + connectors)}} & \DIFaddFL{--- }& \DIFaddFL{--- }& \textbf{\DIFaddFL{\mbox{%DIFAUXCMD
\SI{39.364 \pm 1.773}{Bq}}\hskip0pt%DIFAUXCMD
}} \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item $R_{LI-Fibre}$ = \DIFdelbegin \DIFdel{$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/89.54\;\text{cm}^3\right)}{\left(1138.79\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/89.54\;\text{cm}^3\right)}{\left(12251.08\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ }\DIFaddend = \DIFdelbegin \DIFdel{$8.99 \times 10^4$
    }\DIFdelend \DIFaddbegin \DIFadd{$8.36 \times 10^3$
    }\DIFaddend \item $R_{Fibre-end}$ = $\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(1\;\text{fibre end}/94\;\text{cm}^3\right)}{\left(522\;\text{fibre ends}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $1.87 \times 10^6$
    \DIFaddbegin \item \DIFadd{$R_{LI-Fibre}$ = $\frac{61\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/96.92\;\text{cm}^3\right)}{\left(12251.08\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $1.47 \times 10^4$
    }\item \DIFadd{$R_{Fibre-end}$ = $\frac{61\;\text{days}}{90\;\text{days}}\cdot \frac{\left(1\;\text{fibre end}/87.57\;\text{cm}^3\right)}{\left(522\;\text{fibre ends}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $3.82 \times 10^6$
}\DIFaddend \end{itemize}

\subsection{Gadolinium water}\label{RCalcGd}
\begin{itemize}
    \item $R_{LI-Fibre}$ = \DIFdelbegin \DIFdel{$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/104.74\;\text{cm}^3\right)}{\left(1138.79\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/104.74\;\text{cm}^3\right)}{\left(12251.08\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ }\DIFaddend = \DIFdelbegin \DIFdel{$7.69 \times 10^4$
    }\DIFdelend \DIFaddbegin \DIFadd{$7.15 \times 10^3$
    }\DIFaddend \item $R_{Fibre-end}$ = $\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(1\;\text{fibre end}/94.8\;\text{cm}^3\right)}{\left(522\;\text{fibre ends}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $1.85 \times 10^6$
    \DIFaddbegin \item \DIFadd{$R_{LI-Fibre}$ = $\frac{96\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/104.74\;\text{cm}^3\right)}{\left(12251.08\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $2.14 \times 10^4$
    }\item \DIFadd{$R_{Fibre-end}$ = $\frac{96\;\text{days}}{90\;\text{days}}\cdot \frac{\left(1\;\text{fibre end}/94.8\;\text{cm}^3\right)}{\left(522\;\text{fibre ends}/2.58\times10^{11}\;\text{cm}^3\right)}$ = $5.56 \times 10^6$
}\DIFaddend \end{itemize}
