...
%DIF ADD ../HKLIFibreTN.tex                     Tue Sep 30 15:45:48 2025
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{placeins}
\usepackage{array}
...
\DIFaddbegin \item \DIFadd{v3.5 - Minor revision to answer some comments from review committee. Correct connector ends of OD collimator fibres to FC/PC. Added Yassine's section on initial soak testing results. Add table of laser specifications for clarity.
...
\begin{tabular}{lccc}
\toprule
\DIFaddFL{Model no.	   }& \DIFaddFL{\mbox{%DIFAUXCMD
$\lambda$
}%DIFAUXCMD
}[\DIFaddFL{nm}]  & \DIFaddFL{Pulse width (FWHM) }[\DIFaddFL{ps}] &  \DIFaddFL{\mbox{%DIFAUXCMD
$E_\text{pulse}$
}%DIFAUXCMD
}[\DIFaddFL{pJ}]	\\ \midrule
...
C. The procedure above was be repeated for another sample of the same material in a solution of 0.2\% Gd\mbox{%DIFAUXCMD
$_2$
}%DIFAUXCMD
(SO\mbox{%DIFAUXCMD
$_4$
...
is the expected attenuation length in }\hk \DIFadd{and \mbox{%DIFAUXCMD
$w(\lambda)$
}%DIFAUXCMD
the weights.
}
...
\DIFadd{The criteria for success of a material soak test is defined to be that the effective \mbox{%DIFAUXCMD
$1/e$
}%DIFAUXCMD
attenuation length is greater than 2000~m.
}

\DIFadd{Soak tests of materials are performed at the University of Sheffield, where there is a source of ultra-pure deionised water, Gd\mbox{%DIFAUXCMD
$_2$
}%DIFAUXCMD
(SO\mbox{%DIFAUXCMD
$_4$
...
O powder, a Shimadzu UV-2700i UV-VIS spectrophotometer, and a 102~L constant environment chamber.
}

\subsubsection{\DIFadd{Results}}

\DIFadd{The expected effective attenuation length in }\hk \DIFadd{is calculated using the factor \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
and the UV-VIS cell length of \mbox{%DIFAUXCMD
$x=9$
}%DIFAUXCMD
~cm. Given the worst-case-scenario assumptions in the calculation of \mbox{%DIFAUXCMD
$R$
}%DIFAUXCMD
, it was found that the effective attenuation length for both samples were above the 2~km limit set by }\hk\DIFadd{. Details on the calculation of the factor R for each sample soak tested in UPW can be found in Appendix \ref{RCalcUPW} and in gadolinium-doped water in Appendix \ref{RCalcGd}.
}
...
\DIFadd{The value used for \mbox{%DIFAUXCMD
$Vol_{HK}$
}%DIFAUXCMD
will be the same throughout all calculations and is obtained using : \mbox{%DIFAUXCMD
$Vol_{HK}\;=\; \pi\times3400\times3400\times7100\;\approx\; 2.58\times10^{11}\;\text{cm}^3$
}%DIFAUXCMD
.
}
...
= \mbox{%DIFAUXCMD
$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/89.54\;\text{cm}^3\right)}{\left(1138.79\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$
}%DIFAUXCMD
= \mbox{%DIFAUXCMD
$8.99 \times 10^4$
}%DIFAUXCMD
...
= \mbox{%DIFAUXCMD
$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(1\;\text{fibre end}/94\;\text{cm}^3\right)}{\left(522\;\text{fibre ends}/2.58\times10^{11}\;\text{cm}^3\right)}$
}%DIFAUXCMD
= \mbox{%DIFAUXCMD
$1.87 \times 10^6$
}%DIFAUXCMD
...
= \mbox{%DIFAUXCMD
$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(0.1\;\text{m}/104.74\;\text{cm}^3\right)}{\left(1138.79\;\text{m}/2.58\times10^{11}\;\text{cm}^3\right)}$
}%DIFAUXCMD
= \mbox{%DIFAUXCMD
$7.69 \times 10^4$
}%DIFAUXCMD
...
= \mbox{%DIFAUXCMD
$\frac{32\;\text{days}}{90\;\text{days}}\cdot \frac{\left(1\;\text{fibre end}/94.8\;\text{cm}^3\right)}{\left(522\;\text{fibre ends}/2.58\times10^{11}\;\text{cm}^3\right)}$
}%DIFAUXCMD
= \mbox{%DIFAUXCMD
$1.85 \times 10^6$
}%DIFAUXCMD
...
80			&	FC/PC (air)		&	SMA (water)		&	56				&	Barrel		&	Barrel ID (all)\mbox{%DIFAUXCMD
$^\dagger$
}%DIFAUXCMD
\\
...
80			&	FC/PC (air)	&	Bare (water)	&	84				&	Barrel			& Barrel OD diff. (all)\mbox{%DIFAUXCMD
$^\dagger$
}%DIFAUXCMD
\\
